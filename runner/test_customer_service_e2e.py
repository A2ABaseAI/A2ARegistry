...
def register_customer_service_agent():
    ...
    try:
        ...
        from a2a_reg_sdk.models import (
            AgentBuilder, AgentCardSpecBuilder, SecuritySchemeBuilder,
            AgentInterfaceBuilder, AgentCapabilitiesBuilder, AgentSkillBuilder
        )
        ...
        card = (
            AgentCardSpecBuilder(
                ...
                "http://localhost:9010/api/agent", "1.0.0"
            )
        ...
        agent = (
            AgentBuilder(
                "Customer Service Agent", 
                "AI agent for customer service inquiries", 
                "1.0.0", "E-Commerce Platform"
            )
...
def register_fedex_tracking_agent():
    ...
    try:
        ...
        skill = (
            ...
            .examples([
                "Track FedEx shipment: 'Track 123456789012'",
                "Where is my FedEx package?",
                "Status of FedEx package 987654321098"
            ])
            .input_modes(["text/plain"])
            .output_modes(["text/plain"])
            .build()
        ...
        card = (
            AgentCardSpecBuilder(
                "FedEx Tracking Agent", 
                "AI agent for tracking FedEx shipments and providing status updates",
                "http://localhost:9007/api/agent", "1.0.0"
            )
        ...
        agent = (
            AgentBuilder(
                "FedEx Tracking Agent", 
                "AI agent for tracking FedEx shipments", 
                "1.0.0", "Logistics Provider"
            )
...
def create_mock_customer_service_agent():
    ...
    async def handle_request(request: dict):
        ...
        if "order" in prompt_lower and "status" in prompt_lower:
            return JSONResponse({
                "output": "Your order #12345 is confirmed and will ship within 1-2 business days.",
                "delegate": None
            })
        ...
        if "track" in prompt_lower or "tracking" in prompt_lower or "where" in prompt_lower:
            ...
            if tracking_number:
                ...
                return JSONResponse({
                    ...
                    "delegate": {
                        "prompt": f"Track package {tracking_number}",
                        "context_overrides": {
                ...
                return JSONResponse({
                    "output": "I can help you track your package. Please provide your tracking number.",
                    "delegate": None
                })
        
        # Default response
        return JSONResponse({
            "output": "I'm here to help with your order inquiries. How can I assist you?",
            "delegate": None
        })
    ...
    async def handle_request(request: dict):
        ...
        if tracking_number:
            ...
            if len(tracking_number) == 12:
                ...
                return JSONResponse({
                    "output": f"FedEx Package {tracking_number}: Status not available. Please verify the tracking number."
                })
        else:
            return JSONResponse({
                "output": "Please provide a FedEx tracking number (12 digits)."
            })
...
def test_customer_service_delegation_flow():
    ...
    print("-" * 80)
    cs_thread = create_mock_customer_service_agent()
    fedex_thread = create_mock_fedex_agent()
    ...
    turn2_response = requests.post(
        f"{RUNNER_URL}/host/run",
        json={
            "prompt": "Where is my package? Track 1Z999AA10123456784",
            "token": test_token,
            "force_agent_id": cs_agent_id
        },
        timeout=30
    ...
    turn3_response = requests.post(
        f"{RUNNER_URL}/host/run",
        json={
            "prompt": "Track my FedEx package 123456789012",
            "token": test_token
        },
        timeout=30
    )
    ...
    turn_b = requests.post(
        f"{RUNNER_URL}/host/run",
        json={
            "prompt": "Where is my package? Tracking 1Z999AA10123456784",
            "token": test_token,
            "force_agent_id": cs_agent_id
        },
        timeout=30
    ...
    if turn_b.status_code == 200:
        ...
        has_delegation = (
            "delegated" in str(output_b).lower() or
            ups_agent_id in str(output_b) or
            "ups" in str(output_b).lower() or
            len(output_b) > 500
        )
