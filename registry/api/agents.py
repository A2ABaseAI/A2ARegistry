...
async def delete_agent(agent_id: str, ctx=Depends(require_roles("Administrator", "CatalogManager"))):
    """
    Delete an agent from the registry.

    Requires Administrator or CatalogManager role.
    """
    try:
        # Validate agent_id
        if not agent_id or not isinstance(agent_id, str):
            logger.warning(f"Invalid agent_id provided for deletion: {agent_id}")
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid agent ID")

        tenant_id = ctx.get("tenant") or "default"
        logger.debug(f"Deleting agent {agent_id} for tenant {tenant_id}")
        ...
        logger.error(f"Unexpected error deleting agent {agent_id}: {exc}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error") from exc
